\documentclass[]{hemant-style}
    \begin{document}
        \begin{minipage}[]{\textwidth}
            %name title
           % \fbox{
                \begin{minipage}[t][4em][c]{\textwidth}
                    \Name{HEMANT PUTHRAN}\\
                    +91-7358418843 | hemantjp6@gmail.com\\
                \end{minipage}
                \hrule
                \vspace{1em}
            %}
            %contents
            %\fbox{
                \begin{minipage}{\textwidth}
                   % \fbox{
                        %skills
                        \begin{minipage}[t][][c]{.25\textwidth}
                            \JobDesignation{Opportunities most interested} \\
                            GoLang\\ 
                            Embedded C\\
                            ARM, RTOS\\
                            Control Systems\\
                            Firmware  development\\
                            \newline
                            \SectionHeader{Education} \\
                            \JobDesignation{MTech. IIT-Bombay}\\
                            \ProjectName{Control and Computing \\Dept. of Electrical Engg.}\\
                            \JobDuration{July 2011 - July 2014}\\
                            GPA: 8.04\\
                            \newline
                            \JobDesignation{BTech.VNIT-Nagpur}\\
                            \ProjectName{Electrical Enginering}\\
                            \JobDuration{July 2006 - May 2010}\\
                          %  GPA: 6.61\\
                            \newline
                            \SectionHeader{Tools} \\
                            \ProjectName{Languages}\\
                            C, Go, Javascript, PHP\\
                            \ProjectName{Frameworks}\\
                            ReactJS(16.2.0), React-Redux, ReactPHP,\\
                            WordPress\\
                            \ProjectName{Utilities}\\
                            Git,Webpack\\
                            \ProjectName{Technologies}\\
                            Web-Sockets, XMPP Protocol\\
                            \vspace{1em}
                        \end{minipage}
                    %}
                    %\fbox{
                        %experience
                        \begin{minipage}[t][][c]{.75\textwidth}
                            \SectionHeader{Experience} \\
                            \JobCompany{Freelance}:\JobDuration{March 17 - present}:\JobLocation{Mumbai}\\
                            \JobDesignation{Lead Developer}:\ProjectName{Asterisk Telephony Dashboard}
                            \begin{hitemize}
                                \item Solution provided for dashboard(ReactJS) and backend(ReactPHP)
                                \item Filtering and publishing of Asterisk events generated Bombay AMI and ARI to dashboard
                                \item Event driven non-blocking I/O minimal webserver hosting the client and  providing config and logging utility for admin
                                \item Compelete functionality out of file folder, facilitating easy non technical onsite installation of application
                            \end{hitemize}
                            \JobCompany{E2 Infosystems Private Limited}:\JobDuration{Oct. 15 - March 17}:\JobLocation{Chennai}\\
                            \JobDesignation{Lead Developer}:\ProjectName{Application Redesign and Rewrite}
                            \begin{hitemize}
                                \item Restructuring and Redesign with emphasis on seperation of concerns, reusability, maintainence, feature extensibility of Java based hybrid web app on iOS
                                \item Feature developement in Javascipt and native iOS, leveraging each’s strengths for robustness and flexibility
                                \item Designed primitive protocol and workflow for communication and synchronisation between the iOS logic layer and webapp interface
                                \item Replaced various Java templating frameworks with ReactJS
                                \item Enhanced development workflow and optimal production build generation using Webpack
                            \end{hitemize}
                            \JobDesignation{Lead Developer}:\ProjectName{Web App}
                            \begin{hitemize}
                                \item Sole developer for client(AngularJS) and backend(Laravel)
                                \item Chat feature over web-sockets leveraging XMPP protocol on OpenFire
                                \item Implemented JWT(JSON Web Tokens) for verfiying requests, maintaining user state and progress towards stateless backend
                                \item Built engine to generate SOAP requests, regex XML and parse to JSON for improved response
                            \end{hitemize}
                            \JobDesignation{Lead Developer}:\ProjectName{WordPress App}
                            \begin{hitemize}
                                \item Plugin and Theme Development to manage website data
                                \item Developed rudimentary console to dynamically customize page layout and content, tighly integrated with WordPress API's
                                \item Effective solution by removing maintainence \approx 1200 existing posts
                            \end{hitemize}
                            \JobCompany{Accelya Kale Solutions}:\JobDesignation{iOS Developer}:\JobDuration{June 14 - Oct. 15}:\JobLocation{Mumbai}
                            \begin{hitemize}
                                \item Functional enhancements and fixes to Finesse MBS. Minor fixes to Audit MIS and iBrochures
                            \end{hitemize}
                            \JobCompany{IIT-Bombay}:\JobDesignation{Research Assistant}:\JobDuration{July 11 - June 14}:\JobLocation{Mumbai}
                            \begin{hitemize}
                                \item Designing new experiments and Lab incharge for Masters and Bachelors courses
                                \item Taught and implemented control algorithms using Matlab, DAQ cards and Arduino
                            \end{hitemize}
                            \JobCompany{Enercon India Limited}:\JobDesignation{Field Engineer}:\JobDuration{June 2010 - 11}:\JobLocation{Saundatti, KA}
                            \begin{hitemize}
                                \item Electrical Supervisor designing connecting grid layout, coordinating installation of electric
                                equipment, quality inspection and commissioning of windturbines
                            \end{hitemize}
                            \vspace{1em}
                            \SectionHeader{Academics} \\
                            \JobCompany{Masters Thesis}:\JobDesignation{Polynomial matrix nullspace computation}
                            \begin{hitemize}
                                \item Implemented BlockQR and SVD on GPU using OpenCL and clAMDBLAS
                                \item Block Toeplitz solution implemented in Scilab and in Python using Scipy and improved upon using Row Update QR
                            \end{hitemize}
                            \JobCompany{R \& D project}:\JobDesignation{Embedded Autonomous Quad-Copter Testbed}
                            \begin{hitemize}
                                \item Independently designed and built a Single Axis Test Platform and Thrust Measuring Stand to engineer the quad-copter and test Control algorithms
                                \item Augmented ESC operation resulting in increased operating frequency  and improved granularity in BLDC speed control
                            \end{hitemize}
                            \vspace{1em}
                            \SectionHeader{International Publication} \\
                            \JobCompany{International Deep Drawing Research Group Conference}\\
                            \JobDesignation{Hemant Puthran, Sanket V. Joshi, K. Narasimhan,Forming Behavior of thin foils, p.p. 493-498, 2013, Zurich, Switzerland}
                            \end{minipage}
                    %}
                \end{minipage}
            %}
        \end{minipage}
        
    \end{document}
        